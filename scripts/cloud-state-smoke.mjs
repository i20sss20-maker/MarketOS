...
// Store-level revision semantics.
...
assert.equal(first.clientRevision, 1);
...
const alertOnly = await store.updateAlerts(
  "revision-user",
  [{ id: "server-alert", checked: true }],
);
...
assert.equal(second.clientRevision, 2);
assert.equal(second.serverRevision, 3);
assert.ok(
  second.clientUpdatedAt >=
    firstClientUpdatedAt,
);
...
await assert.rejects(
  ...
  (error) =>
    error instanceof UserStateConflictError &&
    error.currentClientRevision === 2 &&
    error.currentServerRevision === 3,
);

await store.delete("revision-user");
assert.equal(
  await store.get("revision-user"),
  null,
);
...
// API-level revision and conflict semantics.
...
assert.equal(
  Object.keys(
    apiFirst.jsonBody?.state?.drawings ?? {},
  ).length,
  80,
);

const apiGet = await userState(
  apiRequest("GET"),
);

assert.equal(apiGet.status, 200);
assert.equal(
  apiGet.jsonBody?.clientRevision,
  1,
);
assert.equal(
  apiGet.jsonBody?.serverRevision,
  1,
);
...
assert.equal(
  afterWorker.jsonBody?.clientRevision,
  1,
);
assert.equal(
  afterWorker.jsonBody?.serverRevision,
  2,
);
assert.deepEqual(
  afterWorker.jsonBody?.state?.alerts,
  workerUpdate?.payload.alerts,
);

const staleServerPut = await userState(
  apiRequest("PUT", {
    state: sanitized,
    expectedClientRevision: 1,
...
assert.equal(staleServerPut.status, 409);
assert.equal(
  staleServerPut.jsonBody?.conflict,
  true,
);
assert.equal(
...
const safeMergedPut = await userState(
  apiRequest("PUT", {
    state: {
      ...sanitized,
      alerts:
...
assert.equal(safeMergedPut.status, 200);
assert.equal(
  safeMergedPut.jsonBody?.clientRevision,
  2,
);
assert.equal(
  safeMergedPut.jsonBody?.serverRevision,
  3,
);
...
assert.equal(
  deleted.jsonBody?.deleted,
  true,
);

console.log(
  "Cloud Sync V2 smoke passed: auth, drawings, dual revisions, worker-safe merges, conflicts, pagination",
);
