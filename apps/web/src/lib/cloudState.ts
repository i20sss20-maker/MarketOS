...
export function collectLocalCloudState(): CloudStatePayload {
  ...
  for (let index = 0; index < storage.length; index += 1) {
    ...
    const symbolId = key.slice("marketos:drawings:".length);
    if (!symbolId) continue;

    const parsed = parseJson(storage.getItem(key), []);
    if (Array.isArray(parsed)) drawings[symbolId] = parsed;
  }
...
export function applyCloudStateToLocal(state: CloudStatePayload) {
  ...
  for (const [symbolId, drawings] of Object.entries(state.drawings ?? {})) {
    storage.setItem(
