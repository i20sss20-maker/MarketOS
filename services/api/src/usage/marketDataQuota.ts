...
export function configuredMarketDataHardCap(
  ...
  const raw =
    env.MARKET_DATA_DAILY_REQUEST_HARD_CAP?.trim();
  const value =
    raw
      ? Number(raw)
      : NaN;
  ...
  if (
    ...
    throw new ProductionGateError(
      ...
      "Configure MARKET_DATA_DAILY_REQUEST_HARD_CAP between 1 and 100000 before enabling provider-backed market endpoints.",
    );
  }
