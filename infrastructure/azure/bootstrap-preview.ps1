...
az staticwebapp appsettings set --name $AppName --resource-group $ResourceGroup --setting-names MARKET_DATA_PROVIDER=demo MARKET_EVENTS_PROVIDER=demo MARKET_FEED_PROVIDER=demo AI_PROVIDER=local-chart-engine MARKETOS_ENVIRONMENT=azure-preview WEB_PUSH_ENABLED=false --output none
