param(
  ...
  [string]$NotificationsContainer = "notifications"
)
...
Write-Host "Creating/confirming notifications container..." -ForegroundColor Cyan
az cosmosdb sql container create --account-name $CosmosAccount --resource-group $ResourceGroup --database-name $Database --name $NotificationsContainer --partition-key-path "/userId" --throughput 400 --output none
...
az staticwebapp appsettings set --name $StaticWebApp --resource-group $ResourceGroup --setting-names "USER_DATA_PROVIDER=cosmos" "COSMOS_CONNECTION_STRING=$connectionString" "COSMOS_DATABASE=$Database" "COSMOS_CONTAINER=$Container" "COSMOS_NOTIFICATIONS_CONTAINER=$NotificationsContainer" --output none
...
Write-Host "Notifications container: $NotificationsContainer"
